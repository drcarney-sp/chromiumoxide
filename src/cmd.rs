...
impl<T> CommandMessage<T> {
  ...
  pub fn with_session<C: Command>(cmd: C, sender: OneshotSender<T>, session_id: Option<SessionId>) -> serde_json::Result<Self> {
    Ok(Self {
      method: cmd.identifier(),
      session_id,
      params: serde_json::to_value(cmd)?,
...
impl CommandChain {
  ...
  pub fn poll(&mut self, now: Instant) -> NextCommand {
    if let Some((cmd, deadline)) = self.waiting.as_ref() {
      if now > *deadline {
        log::error!("Command {:?} exceeded deadline by {:?}", cmd, now - *deadline);
